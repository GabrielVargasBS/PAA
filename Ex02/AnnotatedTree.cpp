...
public:
    ...
    static double simple_distance(
        ...
        auto insert = [label_dist, get_label](Node* node_bar) { return label_dist("", get_label(node_bar)); };
        auto remove = [label_dist, get_label](Node* node_foo) { return label_dist(get_label(node_foo), ""); };
            ...
            for (int x = 1; x < m; ++x) {
                for (int y = 1; y < n; ++y) {
                    if (fool[i] == fool[x + ioff] && barl[j] == barl[y + joff]) {
                        fd[x][y] = std::min({
                            fd[x - 1][y] + remove_cost(foon[x + ioff]),
